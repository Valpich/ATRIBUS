...
public class AddUeFormTest {
  ...
  private static final String CHAMP_NOM_MATIERE = "nomMatiere";
  ...
  private static final String CHAMP_COEFF = "coeff";
  AddUeForm form;
  ...
  public void testGetResultat(){
    final HttpServletRequest request = Mockito.mock(HttpServletRequest.class); 
    final String nomMatiere = "nomMatiere";
    final String coeff = "-1"; 
    final String semestre = "-1"; 
    
    Mockito.when(request.getParameter(AddUeFormTest.CHAMP_NOM_MATIERE)).thenReturn(nomMatiere);
    Mockito.when(request.getParameter(AddUeFormTest.CHAMP_COEFF)).thenReturn(coeff);
    Mockito.when(request.getParameter(AddUeFormTest.CHAMP_SEMESTRE)).thenReturn(semestre);
    
    assertEquals("echec",form.getResultat()); 
  }
