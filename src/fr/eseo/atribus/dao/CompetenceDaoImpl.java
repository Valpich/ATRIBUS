...
public class CompetenceDaoImpl implements CompetenceDao {
  ...
  /** La constante SQL_UPDATE_NOM_COMPETENCE. */
  private static final String SQL_UPDATE_NOM_COMPETENCE = "UPDATE competence SET nom=? WHERE id=?";
  ...
  /** La constante SQL_SUPPRIMER_PAR_ID. */
  private static final String SQL_SUPPRIMER_PAR_ID = "DELETE FROM competence WHERE id=?";
  ...
  /** La constante SQL_SELECT_FILS_PAR_ID. */
  private static final String SQL_SELECT_FILS_PAR_ID =
      "SELECT * FROM competence WHERE parent_id = ?";
   ...
   * 
   * @see
   * fr.eseo.atribus.dao.CompetenceDao#modifierNomCompetence(fr.eseo.atribus.entities.Competence)
   */
  @Override
  public void modifierNomCompetence(Competence competence) {
    this.update(CompetenceDaoImpl.SQL_UPDATE_NOM_COMPETENCE, competence.getNom(),
        competence.getId());
  }
   ...
   */
  @Override
  public void suppressionRecursive(Competence competence) {
    Competence comp = competence;

    this.supprimerCompetence(comp);
    ...
    try {
      ...
      preparedStatement = DaoUtilitaire.initialisationRequetePreparee(connexion,
          CompetenceDaoImpl.SQL_SELECT_FILS_PAR_ID, false, comp.getId());
      resultSet = preparedStatement.executeQuery();

      while (resultSet.next()) {
        comp = CompetenceDaoImpl.map(resultSet);
        this.suppressionRecursive(comp);
      }
  ...
  private void supprimerCompetence(Competence competence) {
    ...
    try {
      ...
      connexion = this.daoFactory.getConnection();
      preparedStatement = DaoUtilitaire.initialisationRequetePreparee(connexion,
          CompetenceDaoImpl.SQL_SUPPRIMER_PAR_ID, false, competence.getId());
  ...
  /**
   * Méthode pour associer à une UE, une compétence et un niveau.
   *
  ...
  /**
   * Accesseur en lecture du paramètre id ue.
   *
   * @param ue l'ue
   ...
   */
  private void verifierStatut(final int statut) {
    if (statut == 0) {
  ...
  public List<Competence> listeCompetencesUe(UniteEnseignement ue) {
    ...
    try {
      ...
      this.maperCompetences(listeCompetences, preparedStatement);
   ...
   * Maper competences.
   *
   * @param listeCompetences la liste de competences
   * @param preparedStatement le prepared statement
   * @throws SQLException de type SQL exception
   */
  private void maperCompetences(final List<Competence> listeCompetences,
    ...
    while (resultSet.next()) {
      listeCompetences.add(competenceDao.trouverParId(resultSet.getInt(CompetenceDaoImpl.IDCOMP)));
    }
  }
