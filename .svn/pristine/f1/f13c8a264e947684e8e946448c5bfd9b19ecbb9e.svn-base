 ...
 */
public class FiltreEleve implements Filter {
   ...
   * Do filter.
   *
   * @param request ld request
   * @param response ld response
  ...
  public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
     ...
     * connecté.
     */
    if (session.getAttribute(FiltreEleve.ATT_SESSION_USER) == null) {
      reponse.sendRedirect(requete.getContextPath() + FiltreEleve.ACCES_PUBLIC);
    } else {
      @SuppressWarnings("unchecked")
      final List<Utilisateur> listeUtilisateur =
          (List<Utilisateur>) session.getAttribute(FiltreEleve.ATT_SESSION_USER);
