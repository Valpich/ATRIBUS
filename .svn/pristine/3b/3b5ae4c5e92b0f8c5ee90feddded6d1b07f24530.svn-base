 ...
 */
public class FiltreDirecteurEtudes implements Filter {
  ...
  public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
     ...
     * connecté.
     */
    if (session.getAttribute(FiltreDirecteurEtudes.ATT_SESSION_USER) == null) {
      reponse.sendRedirect(requete.getContextPath() + FiltreDirecteurEtudes.ACCES_PUBLIC);
    } else {
      @SuppressWarnings("unchecked")
      final List<Utilisateur> listeUtilisateur =
          (List<Utilisateur>) session.getAttribute(FiltreDirecteurEtudes.ATT_SESSION_USER);
      boolean autorisation = false;
      for (int i = 0; i < listeUtilisateur.size(); i++) {
        if (listeUtilisateur.get(i).getClass().equals(DirecteurEtudes.class)) {
          autorisation = true;
        }
      }
      if (autorisation) {
