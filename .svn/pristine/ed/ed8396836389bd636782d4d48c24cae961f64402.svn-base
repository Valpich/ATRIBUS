...
/**
 * La Classe DirecteurProgrammesDaoImpl.
 */
public class DirecteurProgrammesDaoImpl implements DirecteurProgrammesDao {
  ...
  /** La constante SELECT_1_DP. */
  private static final String SELECT_1_DP =
      "SELECT directeur_programmes.id AS id_directeur_programmes, utilisateur.* ";

  /** La constante SELECT_2_DP. */
  private static final String SELECT_2_DP = " FROM directeur_programmes ";

  /** La constante SELECT_3_DP. */
  private static final String SELECT_3_DP =
      " LEFT JOIN utilisateur ON utilisateur.id = directeur_programmes.id_utilisateur ";

  /** La constante SQL_SELECT_PAR_LOGIN_HASH_DP. */
  private static final String SQL_SELECT_PAR_LOGIN_HASH_DP = SELECT_1_DP + SELECT_2_DP + SELECT_3_DP
      + " WHERE utilisateur.login = ?" + " AND utilisateur.password = ?";

  /** La constante SQL_INSERT_DP. */
  private static final String SQL_INSERT_DP =
      "INSERT INTO directeur_programmes (id_utilisateur) VALUES (?)";

  /** La constante SQL_SELECT_ALL_DP. */
  private static final String SQL_SELECT_ALL_DP = SELECT_1_DP + SELECT_2_DP + SELECT_3_DP;

  /** La constante SQL_SELECT_PAR_ID_UTILISATEUR_DP. */
  private static final String SQL_SELECT_PAR_ID_UTILISATEUR_DP =
      SELECT_1_DP + SELECT_2_DP + SELECT_3_DP + " WHERE id_utilisateur = ?";

  /** La constante SQL_DELETE_PAR_ID_UTILISATEUR_DP. */
  private static final String SQL_DELETE_PAR_ID_UTILISATEUR_DP =
  ...
  public List<DirecteurProgrammes> recupererListe() {
    ...
    try {
      ...
      preparedStatement = DaoUtilitaire.initialisationRequetePreparee(connexion,
          DirecteurProgrammesDaoImpl.SQL_SELECT_ALL_DP, false);
  ...
  public DirecteurProgrammes trouverParLoginHash(String login, String hash) {
    return this.trouver(DirecteurProgrammesDaoImpl.SQL_SELECT_PAR_LOGIN_HASH_DP, login, hash);
  }
  ...
  private DirecteurProgrammes trouverParIdUtilisateur(int idUtilisateur) {
    return this.trouver(DirecteurProgrammesDaoImpl.SQL_SELECT_PAR_ID_UTILISATEUR_DP, idUtilisateur);
  }
  ...
  public DirecteurProgrammes ajouter(DirecteurProgrammes directeurProgrammes) {
    ...
    try {
      ...
      if (this.trouverParIdUtilisateur(idUtilisateur) == null) {
        connexion = this.daoFactory.getConnection();
        preparedStatement = DaoUtilitaire.initialisationRequetePreparee(connexion,
            DirecteurProgrammesDaoImpl.SQL_INSERT_DP, true, idUtilisateur);
  ...
  public int supprimer(DirecteurProgrammes directeurProgrammes) {
    ...
    try {
      ...
      connexion = this.daoFactory.getConnection();
      preparedStatement = DaoUtilitaire.initialisationRequetePreparee(connexion,
          DirecteurProgrammesDaoImpl.SQL_DELETE_PAR_ID_UTILISATEUR_DP, false, idUtilisateur);
