 ...
 */
public class FiltreDirecteurProgrammes implements Filter {
   ...
   * Constructeur.
   */
  public FiltreDirecteurProgrammes() {
    super();
  }

  /**
   * Destructin du filtre.
   *
   * @see Filter#destroy().
   */
  @Override
  ...
  public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
     ...
     * connecté.
     */
    if (session.getAttribute(FiltreDirecteurProgrammes.ATT_SESSION_USER) == null) {
      reponse.sendRedirect(requete.getContextPath() + FiltreDirecteurProgrammes.ACCES_PUBLIC);
    } else {
      @SuppressWarnings("unchecked")
      final List<Utilisateur> listeUtilisateur =
          (List<Utilisateur>) session.getAttribute(FiltreDirecteurProgrammes.ATT_SESSION_USER);
      boolean autorisation = false;
      for (int i = 0; i < listeUtilisateur.size(); i++) {
        if (listeUtilisateur.get(i).getClass().equals(DirecteurProgrammes.class)) {
          autorisation = true;
        }
      }
      if (autorisation) {
