...
/**
 * La classe ModifierCompetenceForm.
 */
public class ModifierCompetenceForm {
  ...
  public void modifierCompetence(HttpServletRequest request) {
    ...
    request.getSession();
    final String nouveauNom = ModifierCompetenceForm.getValeurChamp(request, "nomCompetence");
   ...
   */
  public void supprimerCompetence(HttpServletRequest request) {
    final int idCompetence = Integer.parseInt(request.getParameter("idCompetence"));
    final Competence competence = this.competenceDao.trouverParId(idCompetence);

    try {
      this.competenceDao.suppressionRecursive(competence);
